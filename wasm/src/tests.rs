...
fn test_missing_entrypoint() {
    ...
    let result = run_internal(
        SourceMap::new([("test.qs".into(), code.into())], Some(expr.into())),
        |msg| {
            expect![[r#"{"result":"[{\"document\":\"<project>\",\"diagnostic\":{\"range\":{\"start\":{\"line\":0,\"character\":0},\"end\":{\"line\":0,\"character\":1}},\"message\":\"entry point not found\\n\\nhelp: a single callable with the `@EntryPoint()` attribute must be present if no entry expression is provided and no callable named `Main` is present\",\"severity\":\"error\",\"code\":\"Qsc.EntryPoint.NotFound\"},\"stack\":null}]","success":false,"type":"Result"}"#]].assert_eq(msg);
        },
        1,
    );
...
fn test_run_error_program_multiple_shots() {
    ...
    .expect("code should compile and run");
    expect![[r#"
        {"result":[{"diagnostic":{"code":"Qsc.Eval.QubitUniqueness","message":"runtime error: qubits in invocation are not unique","range":{"end":{"character":49,"line":113},"start":{"character":32,"line":113}},"severity":"error"},"document":"qsharp-library-source:Std/Intrinsic.qs","stack":"Error: qubits in invocation are not unique\nCall stack:\n    at QIR.Intrinsic.__quantum__qis__cx__body in qsharp-library-source:QIR/Intrinsic.qs\n    at Std.Intrinsic.CNOT in qsharp-library-source:Std/Intrinsic.qs\n    at Test.Main in test.qs\n"}],"success":false,"type":"Result"}
        {"result":[{"diagnostic":{"code":"Qsc.Eval.QubitUniqueness","message":"runtime error: qubits in invocation are not unique","range":{"end":{"character":49,"line":113},"start":{"character":32,"line":113}},"severity":"error"},"document":"qsharp-library-source:Std/Intrinsic.qs","stack":"Error: qubits in invocation are not unique\nCall stack:\n    at QIR.Intrinsic.__quantum__qis__cx__body in qsharp-library-source:QIR/Intrinsic.qs\n    at Std.Intrinsic.CNOT in qsharp-library-source:Std/Intrinsic.qs\n    at Test.Main in test.qs\n"}],"success":false,"type":"Result"}
        {"result":[{"diagnostic":{"code":"Qsc.Eval.QubitUniqueness","message":"runtime error: qubits in invocation are not unique","range":{"end":{"character":49,"line":113},"start":{"character":32,"line":113}},"severity":"error"},"document":"qsharp-library-source:Std/Intrinsic.qs","stack":"Error: qubits in invocation are not unique\nCall stack:\n    at QIR.Intrinsic.__quantum__qis__cx__body in qsharp-library-source:QIR/Intrinsic.qs\n    at Std.Intrinsic.CNOT in qsharp-library-source:Std/Intrinsic.qs\n    at Test.Main in test.qs\n"}],"success":false,"type":"Result"}"#]]
    .assert_eq(&output.join("\n"));
}
...
fn test_runtime_error_default_span() {
    ...
    .expect("code should compile and run");
    expect![[r#"{"result":[{"diagnostic":{"code":"Qsc.Eval.UserFail","message":"runtime error: program failed: Cannot allocate qubit array with a negative length","range":{"end":{"character":69,"line":14},"start":{"character":12,"line":14}},"related":[{"location":{"source":"qsharp-library-source:core/qir.qs","span":{"end":{"character":69,"line":14},"start":{"character":12,"line":14}}},"message":"explicit fail"}],"severity":"error"},"document":"qsharp-library-source:core/qir.qs","stack":"Error: program failed: Cannot allocate qubit array with a negative length\nCall stack:\n    at QIR.Runtime.AllocateQubitArray in qsharp-library-source:core/qir.qs\n    at Test.Main in test.qs\n"}],"success":false,"type":"Result"}"#]]
    .assert_eq(&output.join("\n"));
}
